"""max_profile.py - the "Max: The Curse of Brotherhood"-specific half of
vision gameplay: platforming macros, Magic Marker aiming/drawing/erasing,
and the death/draw/destroy counters. The shared loop, LLM plumbing and frame
capture live in gameplay_engine.py.
"""
...
class GameplayMove(BaseModel):
    ...
    action: Literal[
        "move", "jump", "running_jump", "edge_jump_grab", "climb_or_pull_up",
        "swing_and_jump", "draw_marker", "destroy_drawing", "push_pull",
        "interact", "advance_prompt", "wait",
...
class FrameDecision(BaseModel):
    ...
    scene_state: Literal[
        "in_gameplay", "cutscene_or_loading", "death_or_respawn",
        "menu_or_prompt", "checkpoint_reached", "level_complete",
...
# ===========================================================================
# Fallbacks / forced moves / counters
# ===========================================================================
def _initial_counters() -> dict[str, Any]:
    return {"deaths": 0, "draws": 0, "destroys": 0, "stuck_streak": 0}


def _forced_moves(decision: FrameDecision, counters: dict[str, Any]) -> list[GameplayMove] | None:
    if decision.scene_state == "death_or_respawn":
        counters["deaths"] += 1
        return [GameplayMove(action="advance_prompt", button="a",
                             purpose="Respawn at the last checkpoint.")]
    if decision.scene_state in ("cutscene_or_loading", "menu_or_prompt") and not decision.moves:
        return [GameplayMove(action="advance_prompt", button="a",
                             purpose="Dismiss the prompt/cutscene.")]
    return None


def _fallback_moves(decision: FrameDecision) -> list[GameplayMove]:
    heading = decision.progress_direction if decision.progress_direction != "none" else "right"
    return [GameplayMove(
        action="move", direction=heading, duration=0.7,
        purpose="Model returned no move; advance to reveal more.")]


def _on_move_dispatched(move: GameplayMove, counters: dict[str, Any]) -> None:
...
def _print_extra(decision: FrameDecision) -> None:
...
def _cycle_extra_fields(decision: FrameDecision) -> dict[str, Any]:
    return {
...
def _build_terminal_evidence(decision: FrameDecision, cycle: int,
                             frame_path: str | None) -> dict[str, Any]:
    return {"cycle": cycle, "scene_state": decision.scene_state,
            "evidence": decision.checkpoint_evidence, "frame_path": frame_path}


def _extra_session_text(counters: dict[str, Any]) -> str:
    return (f"Deaths so far: {counters.get('deaths', 0)}\n"
            f"Marker strokes drawn so far: {counters.get('draws', 0)} "
            f"(erased {counters.get('destroys', 0)})\n")


def _summarize(counters: dict[str, Any]) -> dict[str, Any]:
    return {
        "marker_draws": counters.get("draws", 0),
        "drawings_destroyed": counters.get("destroys", 0),
...
PROFILE = GameProfile(
    key="max",
    move_model=GameplayMove,
    frame_model=FrameDecision,
    system_prompt=GAMEPLAY_SYSTEM_PROMPT,
    default_goal=("Advance through the level, drawing with the Magic Marker "
                  "where needed, until a checkpoint is reached."),
    artifact_prefix="play",
    json_artifact_name="gameplay-cycles.json",
    execute_move=execute_move,
    success_states={"checkpoint_reached", "level_complete"},
    terminal_flag_key="checkpoint_reached",
    terminal_evidence_key="checkpoint",
